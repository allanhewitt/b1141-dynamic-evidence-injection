...
export class PostgresStore {
  ...
  async closeSession(sessionId){const {rows}=await this.pool.query(`UPDATE activity_sessions SET status='closed',closed_at=COALESCE(closed_at,now()) WHERE id=$1 RETURNING *`,[sessionId]);return rows[0]??null;}
  ...
  async markInformationSeen(sessionId,participantHash){const session=await this.getSession(sessionId);if(!session||session.status!=='open')throw new Error('session is not open');const trace=await this.getTrace(sessionId,participantHash);if(!trace)throw new Error('initial response has not been committed');await this.pool.query(`UPDATE dei_response_traces SET information_seen_at=COALESCE(information_seen_at,now()) WHERE session_id=$1 AND participant_hash=$2`,[sessionId,participantHash]);return this.getTrace(sessionId,participantHash);}
